...
func main () {
	...
	_r1cs, _ := frontend.Compile(ecc.BN254.ScalerField(), r1cs.NewBuilder, &circuit)

	// R1CS implmenets io.WriterTo and io.ReaderFrom
	var buf bytes.Buffer
	_, _ = _r1cs.WriterTo(&buf)

	// gnark objects ( R1CS, providngKey, VerifiyingKey, Proof) must be instantiated
	newR1CS := groth16.NewCS(ecc.BN254)
	_, _ = newR1CS.ReaderFrom(&buf)
